...
void* thread_convolute(void* arg) {
    ...
    for (int row = data->start_row; row < data->end_row; row++) {
        for (int pix = 0; pix < data->src->width; pix++) {
            for (int bit = 0; bit < data->src->bpp; bit++) {
                data->dest->data[Index(pix,row,data->src->width,bit,data->src->bpp)] =
                    getPixelValue(data->src, pix, row, bit, data->algorithm);
            }
        }
    }
...
void convolute_parallel(Image* srcImage, Image* destImage, Matrix algorithm){
    int num_threads = 4;  // adjust if desired
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        if (i == num_threads - 1)
            thread_data[i].end_row = srcImage->height;
        else
            thread_data[i].end_row = (i + 1) * rows_per_thread;

        pthread_create(&threads[i], NULL, thread_convolute, &thread_data[i]);
    }
